...
Slider{
    ...
    property var handleVisible: knobVisible
    property bool areaHovered: false

    onValueChanged: control.knobLabel = "" + Math.floor(control.value/ 60)+ ":" + (control.value % 60).toFixed(0)
    ...
    background: Rectangle {
        ...
        MouseArea{
            ...
            onMouseXChanged: { if (hovered) {/* console.log(mouseX, mouseY);*/ textX = mouseX } }
            onMouseYChanged: { if (hovered) {/* console.log(mouseX, mouseY);*/ textX = mouseX } }
            ...
            onTextXChanged: {
                var res = ((control.to - control.from) * ( (textX) * 100 / control.availableWidth) / 100).toFixed(2)
                var r = Math.floor(res % 60).toString()
                control.knobLabel = "" + Math.floor(res / 60) + ":" + (r.length === 1 ? ("0" + r) : r)
            }
        }
    }
}
