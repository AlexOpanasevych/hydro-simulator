...
void QNanoPainter::drawImage(QNanoImage &image, float x, float y, float alpha)
{
    _checkAlignPixelsAdjust(&x, &y);
    image.setParentPainter(this);
    // Note: This will also update image width & height values
    int id = image.getID(nvgCtx());
    drawImageId(id, x, y, image.width(), image.height(), alpha);
}
...
void QNanoPainter::drawImage(QNanoImage &image, float x, float y, float width, float height, float alpha)
{
    _checkAlignPixelsAdjust(&x, &y);
    _checkAlignPixels(&width, &height);
    image.setParentPainter(this);
    drawImageId(image.getID(nvgCtx()), x, y, width, height, alpha);
}
...
void QNanoPainter::drawImage(QNanoImage &image, const QRectF destinationRect, float alpha)
{
    drawImage(image,
...
void QNanoPainter::drawImage(QNanoImage &image, const QRectF sourceRect, const QRectF destinationRect, float alpha)
{
    image.setParentPainter(this);
    ...
    NVGpaint ip = nvgImagePattern(nvgCtx(), startX, startY, endX, endY, 0.0f, image.getID(nvgCtx()), alpha);
    nvgSave(nvgCtx());
    nvgBeginPath(nvgCtx());
...
void QNanoPainter::drawImageId(int imageId, float x, float y, float width, float height, float alpha)
{
    NVGpaint ip = nvgImagePattern(nvgCtx(), x, y, width, height, 0.0f, imageId, alpha);
    nvgSave(nvgCtx());
    nvgBeginPath(nvgCtx());
    nvgRect(nvgCtx(), x, y, width, height);
    nvgFillPaint(nvgCtx(), ip);
