...
QNanoImage QNanoImage::fromQImage(const QImage &qimage)
{
    QNanoImage image;
    image.m_imageData = new QNanoDataElement();
    image.m_imageData->width = qimage.width();
    image.m_imageData->height = qimage.height();
    image.m_imageData->buffer = (uchar*)qimage.scanLine(0);
    image.m_imageData->imageSize = qimage.bitPlaneCount();
    image.m_textureId = qimage.cacheKey();
    image.updateUniqueKey();
    return image;
}

//QNanoImage QNanoImage::fromQImage(const QImage &image)
//{
//    QNanoImage nanoImage;
//    nanoImage.m_imageData = new QNanoDataElement();
//    nanoImage.m_imageData->width = image.width();
//    nanoImage.m_imageData->height = image.height();
//    nanoImage.m_imageData->buffer = image.bits();
//    nanoImage.m_textureId = image.cacheKey();
//    nanoImage.updateUniqueKey();
...
int QNanoImage::getID(NVGcontext* nvg)
{
    ...
    if (dataCache->contains(m_uniqueKey)) {
        ...
        if(m_imageData->buffer) {
            m_imageData->id = nvgCreateImageRGBA(nvg, m_imageData->width, m_imageData->height, m_flags, m_imageData->buffer);
