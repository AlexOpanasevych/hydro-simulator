...
QNanoColor::QNanoColor(const QString &hex)
{
    setHexColor(hex.toLocal8Bit().constData());
}
...
void QNanoColor::setHexColor(unsigned int hex)
{
    ...
    m_name = rgbToHex(m_nvgColor.r, m_nvgColor.g, m_nvgColor.b);
}
...
void QNanoColor::setHexColor(const char *hex)
{
    ...
    if (length == 7) {
        a = 255;
        r = static_cast<unsigned char>(s.midRef(1,2).toInt(&ok, 16));
        g = static_cast<unsigned char>(s.midRef(3,2).toInt(&ok, 16));
        b = static_cast<unsigned char>(s.midRef(5,2).toInt(&ok, 16));
    } else {
        a = static_cast<unsigned char>(s.midRef(1,2).toInt(&ok, 16));
        r = static_cast<unsigned char>(s.midRef(3,2).toInt(&ok, 16));
        g = static_cast<unsigned char>(s.midRef(5,2).toInt(&ok, 16));
        b = static_cast<unsigned char>(s.midRef(7,2).toInt(&ok, 16));
    }
