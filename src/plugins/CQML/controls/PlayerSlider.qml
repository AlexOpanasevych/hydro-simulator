...
Slider{
    ...
    property var knobLabelText: (value) => {
                                    let min = Math.floor(control.value/ 60)
                                    let sec = (control.value % 60).toFixed(0)
                                    let result = ""
    ...
    onValueChanged: control.knobLabel = knobLabelText(control.value)
    ...
    background: Rectangle {
        ...
        MouseArea{
            ...
            onTextXChanged: {
                control.knobLabel = control.knobLabelText(control.value)
            }
        }
    }
}
